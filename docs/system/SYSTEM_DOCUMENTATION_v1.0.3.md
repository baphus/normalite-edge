...
- **v1.0.3 (2026-08-09):** Retargeted cross-references to the current document versions. No other change.
- **v1.0.2 (2026-08-09):** Corrected the contractor's name to the full legal name, **Josephus Kim L. Sarsonas**, so that the Contract, the accomplishment report, and the Bureau of Internal Revenue documents all carry the same name. No other change. Retargeted cross-references to the current document versions.
- v1.0.1 (2026-08-04): Corrected §9 item 2 — Supabase runs on the **free** tier, which takes no automatic backups at all, so the previous reference to a 7-day Pro-tier window did not apply. Recorded current hosting tiers in §3.1. Retargeted cross-references. **No change to the architecture, security-control, or data-model sections.**
...
| `PROJECT_SUMMARY_v2.0.3.md` | What can the system do? |
...
```
   ...
   All platforms currently run on FREE tiers — hosting cost to CNU is zero.
   Note: the Supabase free tier takes no automatic database backups, and the
   Render free tier spins the API down when idle (cold start on first request).
```
...
| 2 | **No automatic database backups.** Supabase runs on the free tier, which takes none — so there is currently nothing to run a restore drill against. Automatic daily backups require a paid tier. | **Hosting-tier decision (CNU), with cost** | Blocks ISO 27001 A.8.13 and SOC 2 A1.2. More importantly, student examination records are presently unrecoverable if lost. Trade-off set out in `docs/contract/PROPOSAL_AND_SOW_v1.2.0.md` §10.2. |
