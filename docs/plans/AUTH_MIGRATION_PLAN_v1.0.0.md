...
| `pages/auth/LoginPage.tsx` | **Google button primary**; staff password form collapsed behind a secondary link |
| `pages/auth/CompleteProfilePage.tsx` (new) | Reuses `RegisterPage.tsx:17-38` academic fields, minus email/password/confirmPassword |
...
`server/src/utils/jwt.ts` · brute-force `Map` (`auth.service.ts:14-55`) · `client/src/lib/tokenStore.ts` · `client/src/pages/auth/PendingApprovalPage.tsx` · `client/src/pages/auth/RegisterPage.tsx` (fields migrate to `CompleteProfilePage`) · `google-auth-library` dependency · `GOOGLE_CLIENT_SECRET` env var · `users.password_hash` · `users.refresh_token_hash` · refresh cookie handling in `auth.controller.ts`
...
| **4. Client auth** | `lib/supabase.ts`, `AuthContext`, `axios`, `LoginPage`, `CompleteProfilePage` | End-to-end: Google sign-in → profile completion → `ACTIVE` → dashboard |
