...
| **Version** | v1.1.1 |
| **Date** | 2026-08-01 |
| **Status** | Implemented — merged in #24 |
| **Owner** | baphus |
| **Supersedes** | v1.1.0 |
...
|---|---|---|---|
| v1.1.1 | 2026-08-01 | baphus | **Corrects a factual error in v1.0.0/v1.1.0.** Phase 1 instructed "disable email signups" in Supabase. That switch (`disable_signup` / "Allow new users to sign up") is **global, not per-provider** — disabling it also blocks Google sign-ups, which is the primary registration path. Observed in practice as `error_code=signup_disabled` on the first real Google sign-in. Signups now stay enabled and the restriction moved into `AuthService.completeProfile`, which requires a Google identity for any `@cnu.edu.ph` address. Adds `ineligibleReason` to the `/auth/me` contract so the client can distinguish a wrong-domain from a wrong-provider rejection. No decision changed — only the mechanism enforcing D2/D3. |
...
| D3 | **Domain *and provider* enforced in Express**, not via Supabase config | Supabase "Before User Created" hook; `app_metadata` flag; `hd=` param; global signup switch | `public.users` is already the gate, making the hook redundant. `hd=` is a UI hint, **not a security control**. The global signup switch cannot express "email off, Google on" — see §6.7 |
...
### 6.7 Why email signup stays enabled (corrected in v1.1.1)

v1.0.0 and v1.1.0 both instructed: *"disable email signups (admin-create only)"*. **That is not possible in Supabase as stated.**

The setting — "Allow new users to sign up" (`disable_signup` / `GOTRUE_DISABLE_SIGNUP`) — is a **single project-wide switch**, not a per-provider one. Turning it off to close the email path also closes the Google path, which is the primary way every reviewee registers. In practice the first real Google sign-in failed with:
...
**Resolution: leave signups enabled, and enforce the rule where it can actually be expressed** — in `AuthService.completeProfile`, which now requires **both**:

1. the address is `@cnu.edu.ph`, **and**
2. the identity was established through Google (`app_metadata.provider === 'google'`, or `'google'` present in `app_metadata.providers`).

Without (2), re-enabling signups would let anyone register a password against an `@cnu.edu.ph` address they do not own and be handed a `REVIEWEE` account. (2) is what makes (1) meaningful.

This changes no decision — D2 and D3 stand exactly as recorded. Only the enforcement point moves, and it moves *toward* the plan's central principle: `public.users` is the gate, so the rules belong in the code that creates rows there, not in dashboard configuration that can silently drift.

`/auth/me` gains `ineligibleReason` (`'domain' | 'provider' | null`) so the client can explain which rule was broken instead of showing a wrong-domain message to someone whose domain is fine.

**Residual, accepted:** stray `auth.users` rows can now accumulate from email self-signups. They grant nothing — no `public.users` row means no access — so this is the cleanup concern already logged as L5, not a new exposure.
...
| **1. Supabase config** | Enable Google provider; **leave "Allow new users to sign up" ON** (see below); set Site URL and redirect allowlist to the real dev/prod origins; switch to asymmetric JWT keys | Google sign-in returns a valid JWT from a scratch page |
