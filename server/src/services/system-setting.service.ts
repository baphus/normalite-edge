...
export class SystemSettingService {
    async getSettings() {
        const rows = await prisma.$queryRaw<Array<{
            ...
            SELECT enforce_exam_single_tab, tab_switch_grace_seconds, updated_at
        ...
        if (rows.length === 0) {
            await prisma.$executeRaw`
                INSERT INTO system_settings (id, enforce_exam_single_tab, tab_switch_grace_seconds, created_at, updated_at)
                VALUES (${SETTINGS_ROW_ID}, false, 5, NOW(), NOW())
    ...
    async updateSettings(data: { enforceExamSingleTab: boolean; tabSwitchGraceSeconds: number }) {
        const safeTabSwitchGraceSeconds = Math.max(1, Math.min(30, Math.round(Number(data.tabSwitchGraceSeconds || 5))));

        await prisma.$executeRaw`
            INSERT INTO system_settings (id, enforce_exam_single_tab, tab_switch_grace_seconds, created_at, updated_at)
            VALUES (${SETTINGS_ROW_ID}, ${data.enforceExamSingleTab}, ${safeTabSwitchGraceSeconds}, NOW(), NOW())
            ON CONFLICT (id)
            DO UPDATE SET
