...
**Version:** 2.0.3
**Date:** 2026-08-04
**Prepared by:** Josephus Kim L. Sarsonas (independent developer)
**Prepared for:** Cebu Normal University
**Supersedes:** `PROJECT_SUMMARY_v2.0.0.md`, `PROJECT_SUMMARY_v1.1.1.md` (2026-07-07)

## Changelog

- **v2.0.3 (2026-08-09):** Retargeted cross-references to the current document versions. No other change.
- **v2.0.2 (2026-08-09):** Corrected the contractor's name to the full legal name, **Josephus Kim L. Sarsonas**, so that the Contract, the accomplishment report, and the Bureau of Internal Revenue documents all carry the same name. No other change. Retargeted cross-references to the current document versions.
- v2.0.1 (2026-08-04): Retargeted the documentation-set table to the current contract document versions. Recorded current hosting tiers in §2 and added the absence of automatic database backups to §6. **No change to the feature inventory.**
...
For architecture, data model, and security controls, see [docs/system/SYSTEM_DOCUMENTATION_v1.0.3.md](docs/system/SYSTEM_DOCUMENTATION_v1.0.3.md).
...
| Hosting | Vercel (frontend), Render (backend) — **all platforms currently on free tiers; hosting cost ₱0** |
...
1. **No documented data retention or deletion policy** for student PII, exam attempts, or audit logs. This is an institutional policy decision for CNU, not a code defect — and the highest-priority *documentation* gap.
2. **No automatic database backups.** Supabase runs on the free tier, which takes none — so student examination records (attempts, answers, scores) currently have no automatic backup, and there is nothing to run a restore drill against. Automatic daily backups with a retention window require a paid tier. This is a CNU decision with a cost attached, not a code defect; the trade-off is set out in `docs/contract/PROPOSAL_AND_SOW_v1.2.0.md` §10.2. **Of the items in this list, this is the one with the largest downside if left unaddressed** — every other entry is a documentation or process gap, whereas this one risks unrecoverable data.
...
Items 1, 5, and 7 are policy artefacts owned by CNU. Item 2 is a CNU hosting-tier decision with a cost attached — I advise, CNU decides. Items 3, 4, and 6 are engineering/operational work that can be scoped separately.

## 7. Documentation set

| Document | Contents |
|---|---|
| [docs/system/SYSTEM_DOCUMENTATION_v1.0.3.md](docs/system/SYSTEM_DOCUMENTATION_v1.0.3.md) | Architecture, data model, API surface, security controls, standards-readiness |
| This document | Feature inventory and current state |
| [docs/contract/PROPOSAL_AND_SOW_v1.2.0.md](docs/contract/PROPOSAL_AND_SOW_v1.2.0.md) | Contract of Service proposal and statement of work |
| [docs/contract/DELIVERABLES_REGISTER_v1.1.2.md](docs/contract/DELIVERABLES_REGISTER_v1.1.2.md) | Itemised deliverables with acceptance evidence |
| [docs/contract/MAINTENANCE_AND_SUPPORT_TERMS_v1.0.3.md](docs/contract/MAINTENANCE_AND_SUPPORT_TERMS_v1.0.3.md) | Two-month defect-support terms |
| [docs/contract/ADMIN_ORIENTATION_PLAN_v1.0.3.md](docs/contract/ADMIN_ORIENTATION_PLAN_v1.0.3.md) | Administrator orientation programme |
| [docs/manuals/USER_MANUAL_ADMIN_v1.0.3.md](docs/manuals/USER_MANUAL_ADMIN_v1.0.3.md) | Administrator manual |
| [docs/manuals/USER_MANUAL_REVIEWER_v1.0.3.md](docs/manuals/USER_MANUAL_REVIEWER_v1.0.3.md) | Reviewer/instructor manual |
